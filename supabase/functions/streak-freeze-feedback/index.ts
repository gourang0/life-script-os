...
serve(async (req) => {
  ...
  try {
    ...
    const reason_category = String(body.reason_category ?? '').slice(0, 100);
    const reason_details = String(body.reason_details ?? '').slice(0, 500);
    const was_genuine = Boolean(body.was_genuine);
    const streak_days = Math.max(0, Math.min(10000, parseInt(body.streak_days) || 0));
