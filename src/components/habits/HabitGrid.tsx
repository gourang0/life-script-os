...
export const HabitGrid = ({ habits, daysInMonth, onToggle, onDelete }: HabitGridProps) => {
  ...
  return (
    <div className="bg-card rounded-lg p-4 shadow-md border border-border overflow-x-auto">
      <h3 className="font-semibold text-lg mb-4 text-foreground">Daily Habits</h3>
      
      <div className="min-w-[900px]">
        {/* Header with weeks */}
        <div className="grid gap-0 border border-border" style={{ gridTemplateColumns: `180px 60px 40px repeat(${weeks}, 1fr)` }}>
          {/* Header Row */}
          <div className="bg-primary/20 p-2 border-r border-border font-medium text-sm text-foreground">
            DAILY HABITS
          </div>
          <div className="bg-primary/20 p-2 border-r border-border font-medium text-sm text-foreground text-center">
            GOALS
          </div>
          <div className="bg-primary/20 p-2 border-r border-border"></div>
          {Array.from({ length: weeks }, (_, i) => (
            <div key={i} className="bg-primary/20 border-r border-border last:border-r-0">
              <div className="text-center font-medium text-sm text-foreground p-1 border-b border-border">
                WEEK {i + 1}
              </div>
        ...
        {habits.map((habit) => (
          <div
            ...
            style={{ gridTemplateColumns: `180px 60px 40px repeat(${weeks}, 1fr)` }}
          >
            ...
            <div className="flex items-center justify-center border-r border-border text-sm font-medium text-muted-foreground">
              {habit.goal}
            </div>
              ...
              <div key={weekIndex} className="grid grid-cols-7 border-r border-border last:border-r-0">
                {getWeekDays(weekIndex).map((day) => {
                  ...
                  return (
                    <div 
                      key={day} 
                      className={cn(
                        "flex items-center justify-center p-1 border-r border-border last:border-r-0 min-h-[32px]",
                ...
                {weekIndex === weeks - 1 && 
                  Array.from({ length: 7 - getWeekDays(weekIndex).length }, (_, i) => (
                    <div key={`empty-${i}`} className="border-r border-border last:border-r-0 min-h-[32px]" />
                  ))
                }
              </div>
            ))}
